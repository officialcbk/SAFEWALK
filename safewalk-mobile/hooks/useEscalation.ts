...
// 5-stage escalation state machine with auto-progression; stage 4 waits for the
// user to act (call 911 or cancel), stage 5 waits for the user to confirm safe.
...
export function useEscalation(): UseEscalationReturn {
  ...
  // Re-arm timers whenever the active stage changes. armStageTimers sets
  // secondsRemaining synchronously as part of arming the countdown display —
  // this is synchronizing with the timer it starts, not mirroring props into
  // state, so the setState-in-effect rule doesn't apply here.
  useEffect(() => {
